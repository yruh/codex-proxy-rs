...
同完成时间使用同一累计值。同账号的本地同步记录按发生时间合并，排除标记为不计入的记录；本地记录不生成额度观测点。历史点抽样不缩减区间内的累计用量，接口不返回原始 Provider 文档。
...
已加入（或保留的同账号本地记录已开始）且没有已知采样断点的情况下使用累计估算；采集起点不证明历史完整。中途加入账号可以在新基线之后积累，无需等待下次重置。
